...
const mockPostData = () => {
	vi.mock('../../services/post.api', () => {
		const getPost = vi.fn();
		getPost.mockResolvedValue(Promise.resolve({
			id: 1,
			userId: 2,
		...
		getPostComments.mockResolvedValue(Promise.resolve([
			{
				postId: 1,
				id: 1,
				name: 'argh..',
...
const mockUser = (resolved: boolean = true) => {
	...
	vi.mock('../../user.api', () => {
		const getUser = vi.fn();
		resolved
			? getUser.mockResolvedValue(Promise.resolve({
				id: 1,
				name: 'Jane',
				username: 'jane',
...
describe('PostPreview', () => {
	it('should display data', async () => {
		...
		await waitFor(() => expect(screen.getByText('You must read this post')).toBeDefined());
	});
